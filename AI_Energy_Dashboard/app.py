...
def run_ai_energy_analysis(df):
    ...
    # Ensure the columns exist and are properly calculated
...
if "df_analyzed" in st.session_state:
    ...
    with c1:
        ...
        if "Energy_kWh" in df2_plot.columns and "Predicted_Energy" in df2_plot.columns:
            st.line_chart(df2_plot[["Energy_kWh", "Predicted_Energy"]])
        else:
            st.error("Required columns for energy plotting are missing.")
    with c2:
        st.markdown("#### 💰 Cost (Actual vs Predicted)")
        if "Cost_INR" in df2_plot.columns and "Predicted_Cost" in df2_plot.columns:
            st.line_chart(df2_plot[["Cost_INR", "Predicted_Cost"]])
        else:
            st.error("Required columns for cost plotting are missing.")
